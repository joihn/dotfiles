...
-- Use one unnamed register backed by the system clipboard.
-- macOS and graphical Linux use Neovim's native provider (pbcopy, Wayland, or X11).
-- Headless Linux uses OSC 52. Herdr currently forwards OSC 52 writes but not reads,
-- so keep the last Neovim copy locally for p/P instead of waiting for a query timeout.
local system_name = vim.uv.os_uname().sysname
local has_graphical_clipboard = system_name == 'Darwin' or vim.env.WAYLAND_DISPLAY ~= nil or vim.env.DISPLAY ~= nil

if not has_graphical_clipboard then
  if vim.env.HERDR_PANE_ID then
    ...
    vim.g.clipboard = {
      name = 'OSC 52 write with Herdr paste cache',
      copy = { ['+'] = copy, ['*'] = copy },
      paste = { ['+'] = paste, ['*'] = paste },
      cache_enabled = 0,
    }
...
-- PyCharm-like navigation keymaps (Cmd+[ and Cmd+] via kitty keyboard protocol)
-- Navigate backward and forward through cursor positions
vim.keymap.set('n', '<D-[>', '<C-o>', { desc = 'Navigate back to previous cursor position' })
vim.keymap.set('n', '<D-]>', '<C-i>', { desc = 'Navigate forward to next cursor position' })

-- Smart go to definition/implementation function
-- If cursor is at a definition, go to implementation (or references as fallback); otherwise go to definition
local function smart_goto_definition_or_implementation()
  local params = vim.lsp.util.make_position_params()

  -- Request definitions for the symbol under cursor
  vim.lsp.buf_request(0, 'textDocument/definition', params, function(err, result)
    if err or not result or vim.tbl_isempty(result) then
      -- No definitions found, try anyway (might show error to user)
    ...
    local current_line = cursor[1] - 1 -- LSP uses 0-indexed lines

    -- Check if we're currently at a definition location
    local at_definition = false
    for _, def in ipairs(definitions) do
      local def_uri = def.uri or def.targetUri
      local def_range = def.range or def.targetRange

      if def_uri and def_range and def_uri == current_uri then
    ...
    if at_definition then
      -- We're at the definition, try implementation or fall back to references
      -- Check if any LSP client supports implementation
      ...
      if has_implementation then
        ...
        -- Fall back to references when implementation isn't supported
        vim.lsp.buf.references()
      end
    else
      -- We're not at the definition, go to definition
      vim.lsp.buf.definition()
    end
  end)
end

-- Global keybindings for go to definition/implementation
-- <D-b> is Neovim's notation for Super/Cmd+B (kitty keyboard protocol)
vim.keymap.set('n', '<D-b>', smart_goto_definition_or_implementation, { desc = 'Go to definition/implementation (Cmd+B)' })

-- Also keep the standard Alt+B mapping for other contexts
vim.keymap.set('n', '<M-b>', smart_goto_definition_or_implementation, { desc = 'Go to definition/implementation (Alt+B)' })

-- Diagnostic: Check LSP status (useful for troubleshooting)
...
-- 'unnamedplus' routes to the configured clipboard provider.
...
require('lazy').setup({
  ...
  { -- Highlight, edit, and navigate code
    ...
    build = ':TSUpdate',
    config = function()
      -- Configure nvim-treesitter properly
